...
/*
 *  file:     main.cpp (PWM)
 *  brief:    Multi-board PWM example for NVCM
 *  version:  1.00
 *  date:     2019-04-08
 *  authors:  nvitya
*/
...
extern "C" __attribute__((noreturn)) void _start(void)
{
  ...
  unsigned clockspeed = MCU_CLOCK_SPEED;
	...
	// on STM32 the channels on the same timer will have the same base frequency
	// for the proper operation we have to set the same frequency for them
	pwm1.SetFrequency(200000);
	pwm2.SetFrequency(200000);
	pwm3.SetFrequency(200000);

	pwm1.SetOnClocks(pwm1.periodclocks >> 1);
	pwm2.SetOnClocks(pwm2.periodclocks >> 2);
	pwm3.SetOnClocks(pwm3.periodclocks >> 3);
